...
bool IsPathSafe(const string &rootdir, const string &testfile) {
  ...
  // STEP 1
  // Since we don't wnat to deal with "." and "..", realpath allows us to 
  // use absolute paths.
  ...
  if (strlen(testfilebuffer) <= strlen(rootpathbuffer))
    return false;
  if (strncmp(rootpathbuffer, testfilebuffer, strlen(rootpathbuffer)) != 0)
    return false;
...
bool ConnectToServer(const string &hostname, uint16_t portnum, int *client_fd) {
