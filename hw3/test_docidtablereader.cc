...
TEST(Test_DocIDTableReader, TestDocIDTableReaderBasic) {
  ...
  ASSERT_NE(static_cast<DocIDTableReader *>(nullptr), ditr);
  std::list<DocIDElementHeader> docidlist = ditr->GetDocIDList();
  ASSERT_EQ(terminal_results.size(), docidlist.size());
  for (auto iter = docidlist.begin(); iter != docidlist.end(); iter++) {
    ASSERT_EQ(1U, terminal_results.count(iter->docID));
    ASSERT_EQ(terminal_results[iter->docID], iter->numPositions);
  }
  // Clean up.
  delete ditr;
  ...
  ASSERT_NE(static_cast<DocIDTableReader *>(nullptr), ditr);
  docidlist = ditr->GetDocIDList();
  ASSERT_EQ(sun_results.size(), docidlist.size());
  for (auto iter = docidlist.begin(); iter != docidlist.end(); iter++) {
    ASSERT_EQ(1U, sun_results.count(iter->docID));
    ASSERT_EQ(sun_results[iter->docID], iter->numPositions);
  }
  // Clean up.
  delete ditr;
  ...
  ASSERT_NE(static_cast<DocIDTableReader *>(nullptr), ditr);
  docidlist = ditr->GetDocIDList();
  ASSERT_EQ(careful_results.size(), docidlist.size());
  for (auto iter = docidlist.begin(); iter != docidlist.end(); iter++) {
    ASSERT_EQ(1U, careful_results.count(iter->docID));
    ASSERT_EQ(careful_results[iter->docID], iter->numPositions);
  }
  // Clean up.
  delete ditr;
